...
#define  GRY_X_offset	56.7														//陀螺仪的零点漂

///////////////////////////////////////////////////////////////////////////
s16  temp_acc_y=0,temp_acc_z=0,temp_Gry_x=0;				   	         //计算倾角中间变量，读出的数据为整形的
float  Acc_angle=0,Gry_vivi=0;												//加速度的倾角计算得出的结果为浮点类型；
...
{
	MPU_IIC_Start();
	MPU_IIC_Send_Byte(SlaveAddress+0);
	MPU_IIC_Wait_Ack();
...
{
	MPU_IIC_Init();
	if(Single_ReadIIC(WHO_AM_I)==0X68)
	{
		Single_WriteIIC(PWR_MGMT_1, 0x00);										//电源管理1，解除休眠状态，时钟为内部8MHz
		Single_WriteIIC(SMPLRT_DIV, 0x07);										//采样速率125Hz
...
void Read_Gry_x()
{
	...
	Gry_vivi =((float)temp_Gry_x)*(0.030517);
}
//////////////////////////////////////
void Filter(void)
{
  ...
  Read_Gry_x();																					    //读取陀螺仪的值
	Kalman_Filter(Gry_vivi,Acc_angle);														 //卡尔曼滤波
	printf("陀螺仪 %f  \r\n     加速计 %f\r\n",Gry_vivi,Acc_angle);
}
